...
resource_comment = Table(
    ...
    Column("approval_user_id", Text, ForeignKey("user.id", onupdate='CASCADE', ondelete='CASCADE'))
)
...
resource_comment_reply = Table(
    ...
    metadata,
    Column("id", Text, primary_key=True, nullable=False),
    Column("resource_comment_id", Text, ForeignKey("resource_comment.id", onupdate='CASCADE', ondelete='CASCADE'), nullable=False),
    Column("content", Text),
    Column("created", TIMESTAMP),
    Column("creator_user_id", Text, ForeignKey("user.id", onupdate='CASCADE', ondelete='CASCADE'))
)
...
resource_comment_summary = Table(
    ...
    metadata,
    Column("id", Text, primary_key=True, nullable=False),
    Column("resource_id", Text, ForeignKey("resource.id", onupdate='CASCADE', ondelete='CASCADE'), nullable=False),
