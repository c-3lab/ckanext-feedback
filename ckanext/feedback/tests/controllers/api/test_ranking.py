...
class TestRankingApi:
    ...
    def test_calculate_date_range_from_period_validation_called(
        ...
        mock_validate_period_months_ago.assert_called_once_with(period_months_ago)
    ...
    def test_get_dataset_download_ranking_with_feedback_config_and_municipality(
        self, mock_get_download_ranking, mock_config_get
    ):
        is_feedback_config = True
        enable_orgs = []
        top_ranked_limit = '5'
        start_year_month = '2006-10'
        end_year_month = '2023-12'
        municipality_name = None
    ...
    def test_get_dataset_download_ranking_with_municipality(
        self, mock_get_download_ranking, mock_config_get
    ):
        is_feedback_config = True
        enable_orgs = []
    ...
    def test_validate_start_year_month(self):
        pattern = r"^\d{4}-(0[1-9]|1[0-2])$"
        start_year_month = '2024-1'

        with pytest.raises(ValidationError) as exc_info:
            DatasetRankingController.validate_start_year_month(
    ...
    def test_validate_end_year_month(self):
        pattern = r"^\d{4}-(0[1-9]|1[0-2])$"
        end_year_month = '2024-1'

        with pytest.raises(ValidationError) as exc_info:
            DatasetRankingController.validate_end_year_month(pattern, end_year_month)
    ...
    def test_validate_period_months_ago(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            DatasetRankingController.validate_period_months_ago(period_months_ago)
    ...
    def test_validate_enable_orgs(self):
        enable_orgs = None

        with pytest.raises(ValidationError) as exc_info:
            DatasetRankingController.validate_enable_orgs(enable_orgs)
    ...
    def test_validate_municipality_name(self):
        municipality_name = 'test_org3'
        enable_org = ['test_org1', 'test_org2']

        with pytest.raises(ValidationError) as exc_info:
            DatasetRankingController.validate_municipality_name(
                municipality_name, enable_org
            )
        ...
        assert (
            error_message
            == "A municipality with the download feature disabled has been selected."
        )
    ...
    def test_validate_download_function(self, mock_feedback_config):
        ...
        assert error_message == "Download function is off."
